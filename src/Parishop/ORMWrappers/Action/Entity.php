...
class Entity extends \Parishop\ORMWrappers\EntitySemantic
{
    ...
    {
        $this->requireProductDiscounts();
        if(!array_key_exists($optionId, $this->productDiscounts)) {
            $this->productDiscounts[$optionId] = 0;
        }

        return $this->productDiscounts[$optionId];
    }
    ...
    public function url($url = array(), $query = array(), $resolverPath = 'app.action')
    {
        return parent::url(
            array_merge(
                array(
    ...
    private function requireProductDiscounts()
    {
        ...
        foreach($this->actionProducts() as $actionProduct) {
            $this->productDiscounts[$actionProduct->optionId] = $actionProduct->discount;
        }
    }
