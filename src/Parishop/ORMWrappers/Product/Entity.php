 ...
 * @method \PHPixie\ORM\Loaders\Loader\Proxy\Editable|\Parishop\ORMWrappers\AllowAction\Entity[] actions()
 ...
 * @method \PHPixie\ORM\Loaders\Loader\Proxy\Editable|\Parishop\ORMWrappers\Option\Entity[] optionsRests()
 * @method \PHPixie\ORM\Loaders\Loader\Proxy\Editable|\Parishop\ORMWrappers\RestOption\Entity[] restOptions()
...
class Entity extends \Parishop\ORMWrappers\Entity
{
    ...
    {
        $this->requireActionDiscounts();
        if(array_key_exists($actionId, $this->actionDiscounts)) {
            return $this->actionDiscounts[$actionId];
        }
    ...
    {
        $old_price = $this->option()->special ? $this->option()->old_price : 0;

        return $old_price > $this->price() ? (int)$old_price : null;
    }
    ...
    public function related()
    {
        ...
        $query->join($this->builder->components()->orm()->repository('restOption')->config()->get('table'));
        $query->on('products.id', 'restOptions.productId');
        ...
        $query->endGroup();
        $query->whereNot('products.par', 'like', '%#%');
        $query->groupBy('products.id');

        return $this->builder->components()->orm()->product->in($query->offset(0)->limit(16)->execute()->getField('id'))->find();
    }

    public function rests($optionId = null, $maxQuantity = null)
    {
        if($this->rests === null) {
            $this->rests = array();
            foreach($this->restOptions() as $restOption) {
                $quantity = 0;
                if(!empty($this->rests[$restOption->optionId])) {
                    $quantity = $this->rests[$restOption->optionId];
                }
                $this->rests[$restOption->optionId] = $restOption->quantity + $quantity;
            }
            foreach($this->rests as $id => $quantity) {
        ...
        if($optionId === null) {
            return array_sum($this->rests);
        }
        if(array_key_exists($optionId, $this->rests)) {
            return $this->rests[$optionId];
        }
    ...
    {
        $query = $this->builder->components()->database()->get()->selectQuery();
        $query->fields(array('products.id'));
        $query->table(strtolower('allowProducts'), 'products');
        $query->join(strtolower('allowOptions'), 'allowOptions');
        $query->on('allowOptions.productId', 'products.id');
        $query->where('tv_id', $this->tv_id);
        $tvIds = $query->execute()->getField('id');

        return $this->builder->components()->orm()->product->in($tvIds)->find(
            array(
                'mainColor',
                'productImages',
                'options.size',
    ...
    private function requireActionDiscounts()
    {
        if($this->actionDiscounts !== null) {
            return;
        }
        $this->actionDiscounts = array();
        foreach($this->actionProducts() as $actionProduct) {
            $this->actionDiscounts[$actionProduct->actionId] = $actionProduct->discount;
        }
    }
